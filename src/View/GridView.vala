...
public class Files.GridView : Gtk.Widget, Files.ViewInterface {
    ...
    construct {
        var model = new ListStore (typeof (Files.File));
        var selection_model = new Gtk.MultiSelection (model);
        var filter = new Gtk.FileFilter ();
        var filter_model = new Gtk.FilterListModel (selection_model, filter);
    ...
    //     grid_view.set_drag_dest_item (path, Gtk.IconViewDropPosition.DROP_INTO);
    // }
    ...
    //     return grid_view.get_path_at_pos (win_x, win_y + (int)(scrolled_window.get_vadjustment ().get_value ()));
    // }

    // public override void grid_view_select_all () {
    //     grid_view.select_all ();
    // }

    // public override void grid_view_unselect_all () {
    //     grid_view.unselect_all ();
    //     previous_linear_selection_path = null;
    //     previous_linear_selection_direction = 0;
    // }

    // public override void grid_view_unselect_others () {
    //     Gtk.TreePath path = null;
    //     grid_view.get_cursor (out path, null);
    //     grid_view.unselect_all ();
    ...
    //     if (path != null) {
    //         grid_view.select_path (path); /* This selects path but does not unselect the rest (unlike grid_viewView) */
    //         most_recently_selected = path.copy ();
    //         if (cursor_follows) {
    //             grid_view.set_cursor (path, null, false);
    //         }
    //     }
    // }
    ...
    //     if (path != null) {
    //         return grid_view.path_is_selected (path);
    //     } else {
    //         return false;
    //     }
    // }
    ...
    //     start_path = null;
    //     end_path = null;
    //     return grid_view.get_visible_range (out start_path, out end_path);
    // }
    ...
    //     grid_view.selected_foreach ((grid_view, path) => {
    ...
    //     grid_view.get_item_at_pos ((int)x, (int)y, out path, out cell_renderer);
    //     zone = (path != null ? ClickZone.BLANK_PATH : ClickZone.BLANK_NO_PATH);

    //     if (cell_renderer != null) {
    //         Gdk.Rectangle rect, area;
    //         grid_view.get_cell_rect (path, cell_renderer, out rect);
    //         area = cell_renderer.get_aligned_area (grid_view, Gtk.CellRendererState.PRELIT, rect);
    ...
    //                  * unlike grid_viewView will not rubberband if clicked on an item. */
    ...
    //     if (grid_view == null || path == null || slot == null || slot.directory == null ||
    //         slot.directory.permission_denied || slot.directory.is_empty ()) {

    //         return;
    //     }

    //     grid_view.scroll_to_path (path, scroll_to_top, 0.5f, 0.5f);
    // }
    ...
    //     grid_view.set_cursor (path, renderer, start_editing);
    // }
    ...
    // public bool move_cursor (uint keyval, bool only_shift_pressed, bool control_pressed) {
    ...
    //     if (!select) {
    //         grid_view.selection_changed.disconnect (on_view_selection_changed);
    //     } else {
    //         select_path (path);
    //     }
    ...
    //     if (!select) {
    //         grid_view.selection_changed.connect (on_view_selection_changed);
    //     }
    // }
    ...
    // // protected override void freeze_grid_view () {
    //     // grid_view_frozen = true;
    //     // grid_view.freeze_child_notify ();
    //     // grid_view.set_model (null);
    // // }

    // // protected override void thaw_grid_view () {
    //     // if (grid_view_frozen) {
    //     //     grid_view.set_model (model);
    //     //     grid_view.thaw_child_notify ();
    //     //     grid_view_frozen = false;
    //     // }
    // // }
    ...
    //     var selected_paths = grid_view.get_selected_items ();
    ...
    //     grid_view.set_cursor (path, null, false);
    //     grid_view.scroll_to_path (path, false, 0.5f, 0.5f);
    // }
    ...
    //     int item_row = grid_view.get_item_row (path);
    //     if (item_row == 0) {
    //         return path;
    //     }
    //     int cols = get_n_cols ();
    ...
    //     if (grid_view.model.get_iter (out iter, new_path)) {
    //         return new_path;
    //     } else {
    //         return path;
    //     }
    ...
    //     int index = 0;
    //     while (grid_view.get_item_row (path) == 0) {
    //         index++;
    //         path.next ();
    //     }
    //     return index;
