...
#if 0
void
marlin_drag_init (MarlinDragInfo     *drag_info,
...
void
marlin_drag_finalize (MarlinDragInfo *drag_info)
{
    gtk_target_list_unref (drag_info->target_list);
    marlin_drag_destroy_selection_list (drag_info->selection_list);
...
{
    return g_new0 (MarlinDragSelectionItem, 1);
}

//TODO replace this
static void
drag_selection_item_destroy (MarlinDragSelectionItem *item)
{
    g_free (item->uri);
    g_free (item);
}
...
void
marlin_drag_destroy_selection_list (GList *list)
{
    GList *p;
...
#if 0
char **
marlin_drag_uri_array_from_selection_list (const GList *selection_list)
{
    GList *uri_list;
    char **uris;

    uri_list = marlin_drag_uri_list_from_selection_list (selection_list);
    uris = marlin_drag_uri_array_from_list (uri_list);
...
GList *
marlin_drag_uri_list_from_selection_list (const GList *selection_list)
{
    MarlinDragSelectionItem *selection_item;
    GList *uri_list;
    const GList *l;

    uri_list = NULL;
    for (l = selection_list; l != NULL; l = l->next) {
        selection_item = (MarlinDragSelectionItem *) l->data;
...
char **
marlin_drag_uri_array_from_list (const GList *uri_list)
{
    const GList *l;
    char **uris;
    int i;
...
GList *
marlin_drag_uri_list_from_array (const char **uris)
{
    GList *uri_list;
    int i;
...
GList *
marlin_drag_build_selection_list (GtkSelectionData *data)
{
    GList *result;
    const guchar *p, *oldp;
    int size;
...
static gboolean
marlin_drag_file_local_internal (const char *target_uri_string,
...
gboolean
marlin_drag_uris_local (const char *target_uri,
                        const GList *source_uri_list)
{
    /* must have at least one item */
    g_assert (source_uri_list);

    return marlin_drag_file_local_internal (target_uri, source_uri_list->data);
}

gboolean
marlin_drag_items_local (const char *target_uri_string,
                         const GList *selection_list)
{
    /* must have at least one item */
    g_assert (selection_list);

    return marlin_drag_file_local_internal (target_uri_string,
                                            ((MarlinDragSelectionItem *)selection_list->data)->uri);
}

#if 0
gboolean
marlin_drag_items_in_trash (const GList *selection_list)
{
     ...
     * and currently dropped items can only be from the same container
     */
    return eel_uri_is_trash (((MarlinDragSelectionItem *)selection_list->data)->uri);
}

gboolean
marlin_drag_items_on_desktop (const GList *selection_list)
{
    char *uri;
     ...
     * and currently dropped items can only be from the same container
     */
    uri = ((MarlinDragSelectionItem *)selection_list->data)->uri;
    if (eel_uri_is_desktop (uri)) {
        return TRUE;
    }

    desktop = marlin_get_desktop_location ();
...
GdkDragAction
marlin_drag_default_drop_action_for_netscape_url (GdkDragContext *context)
{
               ...
               GOFFile *file2)
{
    ...
    if (file1 != NULL && file2 != NULL) {
        id1 = marlin_file_get_filesystem_id (file1);
        id2 = marlin_file_get_filesystem_id (file2);
...
source_is_deletable (GFile *file)
{
    ...
    /* if there's no a cached GOFFile, it returns NULL */
    naut_file = marlin_file_get_existing (file);
    if (naut_file == NULL) {
        return FALSE;
    }

    ret = marlin_file_can_delete (naut_file);
    marlin_file_unref (naut_file);

    return ret;
}

void
marlin_drag_default_drop_action_for_icons (GdkDragContext *context,
                                           ...
                                           int *action)
{
    ...
    GOFFile *dropped_file, *target_file;
    ...
    dropped_uri = ((MarlinDragSelectionItem *)items->data)->uri;
    dropped_file = marlin_file_get_existing_by_uri (dropped_uri);
    target_file = marlin_file_get_existing_by_uri (target_uri_string);
    ...
    if (eel_uri_is_trash (target_uri_string)) {
        ...
        marlin_file_unref (dropped_file);
        marlin_file_unref (target_file);
        return;

    } else if (dropped_file != NULL && marlin_file_is_launcher (dropped_file)) {
        if (actions & GDK_ACTION_MOVE) {
            *action = GDK_ACTION_MOVE;
        }
        marlin_file_unref (dropped_file);
        ...
        target = marlin_get_desktop_location ();

        marlin_file_unref (target_file);
        target_file = marlin_file_get (target);
    ...
    } else if (target_file != NULL && marlin_file_is_archive (target_file)) {
...
GdkDragAction
marlin_drag_default_drop_action_for_uri_list (GdkDragContext *context,
...
gboolean
marlin_drag_drag_data_get (GtkWidget *widget,
                           ...
                           MarlinDragEachSelectedItemIterator each_selected_item_iterator)
{
    ...
    switch (info) {
    case MARLIN_ICON_DND_GNOME_ICON_LIST:
        result = g_string_new (NULL);
        (* each_selected_item_iterator) (add_one_gnome_icon, container_context, result);
        break;

    case MARLIN_ICON_DND_URI_LIST:
    case MARLIN_ICON_DND_TEXT:
        result = g_string_new (NULL);
...
marlin_drag_autoscroll_in_scroll_region (GtkWidget *widget)
{
    ...
    marlin_drag_autoscroll_calculate_delta (widget, &x_scroll_delta, &y_scroll_delta);
...
void
marlin_drag_autoscroll_calculate_delta (GtkWidget *widget, float *x_scroll_delta, float *y_scroll_delta)
{
...
void
marlin_drag_autoscroll_start (MarlinDragInfo *drag_info,
                              GtkWidget        *widget,
                              GtkFunction       callback,
                              gpointer          user_data)
{
    if (marlin_drag_autoscroll_in_scroll_region (widget)) {
...
void
marlin_drag_autoscroll_stop (MarlinDragInfo *drag_info)
{
    if (drag_info->auto_scroll_timeout_id != 0) {
        g_source_remove (drag_info->auto_scroll_timeout_id);
        drag_info->auto_scroll_timeout_id = 0;
...
gboolean
marlin_drag_selection_includes_special_link (GList *selection_list)
{
    GList *node;
    char *uri;

    for (node = selection_list; node != NULL; node = node->next) {
        uri = ((MarlinDragSelectionItem *) node->data)->uri;
                        ...
                        gpointer            user_data)
{
    ...
    g_assert (MARLIN_IS_WINDOW_INFO (window));
    ...
    if (drag_info->target_location != NULL) {
        ...
        if (drag_info->target_slot != NULL) {
            ...
            target_slot = marlin_window_info_get_active_slot (MARLIN_WINDOW_INFO (window));
        }

        if (target_slot != NULL) {
            target_uri = marlin_window_slot_info_get_current_location (target_slot);
        }
    }

    if (drag_info->have_data &&
        drag_info->have_valid_data) {
        if (drag_info->info == MARLIN_ICON_DND_GNOME_ICON_LIST) {
            marlin_drag_default_drop_action_for_icons (context, target_uri,
                                                       drag_info->data.selection_list,
                                                       &action);
        } else if (drag_info->info == MARLIN_ICON_DND_URI_LIST) {
            action = marlin_drag_default_drop_action_for_uri_list (context, target_uri);
        } else if (drag_info->info == MARLIN_ICON_DND_NETSCAPE_URL) {
            action = marlin_drag_default_drop_action_for_netscape_url (context);
        }
    }
...
static void
drag_info_clear (MarlinDragSlotProxyInfo *drag_info)
{
    if (!drag_info->have_data) {
        goto out;
    }
    ...
    if (drag_info->info == MARLIN_ICON_DND_GNOME_ICON_LIST) {
        marlin_drag_destroy_selection_list (drag_info->data.selection_list);
    } else if (drag_info->info == MARLIN_ICON_DND_URI_LIST) {
                        ...
                        MarlinDragSlotProxyInfo *drag_info)
{
    ...
    target_view = NULL;
    if (target_slot != NULL) {
        target_view = marlin_window_slot_info_get_current_view (target_slot);
    }

    if (target_slot != NULL && target_view != NULL) {
        if (drag_info->info == MARLIN_ICON_DND_GNOME_ICON_LIST) {
            uri_list = marlin_drag_uri_list_from_selection_list (drag_info->data.selection_list);
            g_assert (uri_list != NULL);

            marlin_view_drop_proxy_received_uris (target_view,
                                                  uri_list,
                                                  target_uri,
            ...
            marlin_view_drop_proxy_received_uris (target_view,
                                                  ...
                                                  target_uri,
                                                  gdk_drag_context_get_selected_action (context));
        } if (drag_info->info == MARLIN_ICON_DND_NETSCAPE_URL) {
            marlin_view_drop_proxy_received_netscape_url (target_view,
                               ...
                               gpointer            user_data)
{
    ...
    if (info == MARLIN_ICON_DND_GNOME_ICON_LIST) {
        drag_info->data.selection_list = marlin_drag_build_selection_list (data);

        drag_info->have_valid_data = drag_info->data.selection_list != NULL;
    } else if (info == MARLIN_ICON_DND_URI_LIST) {
        uris = gtk_selection_data_get_uris (data);
        drag_info->data.uri_list = marlin_drag_uri_list_from_array ((const char **) uris);
        g_strfreev (uris);

        drag_info->have_valid_data = drag_info->data.uri_list != NULL;
    } else if (info == MARLIN_ICON_DND_NETSCAPE_URL) {
...
void
marlin_drag_slot_proxy_init (GtkWidget *widget,
                             MarlinDragSlotProxyInfo *drag_info)
{
    const GtkTargetEntry targets[] = {
        { MARLIN_ICON_DND_GNOME_ICON_LIST_TYPE, 0, MARLIN_ICON_DND_GNOME_ICON_LIST },
        { MARLIN_ICON_DND_NETSCAPE_URL_TYPE, 0, MARLIN_ICON_DND_NETSCAPE_URL }
    };
    ...
    gtk_target_list_add_uri_targets (target_list, MARLIN_ICON_DND_URI_LIST);
