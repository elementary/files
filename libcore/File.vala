...
public class Files.File : GLib.Object {
    ...
    public ThumbState thumbstate = ThumbState.UNKNOWN;
    ...
    public bool exists {
        ...
        set {
            assert (!value);  // File created existing and can only be set to not existing
            if (!value && _exists) {
    ...
    public Gdk.Pixbuf? get_icon_pixbuf (int size, int scale, IconFlags flags = IconFlags.USE_THUMBNAILS) {
        GLib.return_val_if_fail (size >= 1, null);
        return get_iconinfo (size, scale, flags).get_pixbuf_nodefault ();
    }

    private bool pix_is_final = false;
    // Does not compile if use pix_size and pix_scale as default values for some reason
    public void update_icon (int size = -1, int scale = -1) {
        if (size < 16) {
            size = pix_size;
        }
        ...
        var same_size = (pix_size == size && pix_scale == scale);
        ...
        pix = get_iconinfo (size, scale, IconFlags.USE_THUMBNAILS).get_pixbuf_nodefault ();
        pix_size = size;
        pix_scale = scale;
    }

    private Files.IconInfo get_iconinfo (int size, int scale, Files.File.IconFlags flags) {
        pix_is_final = true;
        ...
        if (custom_icon_name != null) {
            if (GLib.Path.is_absolute (custom_icon_name)) {
                iconinfo = Files.IconInfo.lookup_from_path (custom_icon_name, size, scale);
            } else {
                iconinfo = Files.IconInfo.lookup_from_name (custom_icon_name, size, scale);
            }
        }

        if (iconinfo == null && thumbnail_ready) {
            iconinfo = Files.IconInfo.lookup_from_path (thumbnail_path, size, scale, is_remote);
        }
        ...
        if (iconinfo == null || iconinfo.pixbuf == null) {
            ...
            if (gicon != null) {
                iconinfo = Files.IconInfo.lookup (gicon, size, scale, is_remote);
                if (iconinfo == null || iconinfo.pixbuf == null) {
                    iconinfo = Files.IconInfo.get_generic_icon (size, scale);
                }
            } else {
                iconinfo = Files.IconInfo.get_generic_icon (size, scale);
            }
