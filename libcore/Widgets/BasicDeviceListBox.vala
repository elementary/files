...
public class Sidebar.BasicDeviceListBox : Gtk.Box, Sidebar.SidebarListInterface {
    public Files.SidebarInterface sidebar { get; set construct; }
    public Gtk.ListBox list_box { get; set construct; }

    private VolumeMonitor volume_monitor;

    public BasicDeviceListBox (Files.SidebarInterface sidebar) {
    ...
    construct {
        ...
        list_box.row_activated.connect ((row) => {
            if (row is BasicBookmarkRow) {
                ((BasicBookmarkRow) row).activated ();
            }
        });
    ...
    private int device_sort_func (Gtk.ListBoxRow? row1, Gtk.ListBoxRow? row2) {
        var key1 = (row1 != null && (row1 is BasicAbstractMountableRow)) ? ((BasicAbstractMountableRow)row1).sort_key : "";
        var key2 = (row2 != null && (row2 is BasicAbstractMountableRow)) ? ((BasicAbstractMountableRow)row2).sort_key : "";

        return strcmp (key1, key2);
    }

    private BasicAbstractMountableRow add_bookmark (string label, string uri, Icon gicon,
                                    string? uuid = null,
                                    Drive? drive = null,
                                    Volume? volume = null,
                                    Mount? mount = null,
        ...
        BasicAbstractMountableRow? bm = null; // Existing bookmark with same uuid
        if (!has_uuid (uuid, uri, out bm) || bm.custom_name != label) { //Could be a bind mount with the same uuid
            BasicAbstractMountableRow new_bm;
            if (drive != null && volume == null) {
                new_bm = new BasicDriveRow (
                    label,
                    uri,
                    gicon,
                    this,
                ...
                new_bm = new BasicVolumelessMountRow (
                    label,
                    uri,
                    gicon,
                    this,
    ...
    public override void refresh_info () {
        ...
        while (row != null) {
            if (row is BasicAbstractMountableRow) {
                ((BasicAbstractMountableRow)row).update_free_space ();
            }
    ...
    private bool has_uuid (string? uuid, string? fallback, out BasicAbstractMountableRow? row) {
        ...
        if (searched_uuid != null) {
            ...
            while (child != null) {
                if (child is BasicAbstractMountableRow &&
                    ((BasicAbstractMountableRow) child).uuid == searched_uuid) {

                    row = (BasicAbstractMountableRow) child;
                    return true;
                }
    ...
    public BasicBookmarkRow? add_sidebar_row (string label, string uri, Icon gicon) {
    ...
    public void select_item (Gtk.ListBoxRow? item) {
        if (item != null && item is BasicAbstractMountableRow) {
