...
public class Sidebar.BasicBookmarkListBox : Gtk.Box, Sidebar.SidebarListInterface {
    ...
    public BasicBookmarkListBox (Files.SidebarInterface sidebar) {
    ...
    public BasicBookmarkRow? add_bookmark (string label,
                                               string uri,
                                               Icon gicon,
                                               bool pinned = false,
    ...
    private BasicBookmarkRow? insert_bookmark (string label,
                                                   string uri,
                                                   Icon gicon,
                                                   int index,
                                                   bool pinned = false,
        ...
        var row = new BasicBookmarkRow (label, uri, gicon, this, pinned, permanent);
        if (index >= 0) {
    ...
    public void select_item (Gtk.ListBoxRow? item) {
        if (item != null && item is BasicBookmarkRow) {
    ...
    public override bool add_favorite (string uri,
        ...
        while (row != null && ((BasicBookmarkRow) row).pinned) {
            pinned++;
    ...
    public override bool remove_item_by_id (uint32 id) {
        ...
        while (row != null) {
            if (row is BasicBookmarkRow) {
                unowned var bm = (BasicBookmarkRow)row;
    ...
    public virtual bool is_drop_target () { return false; }
}
