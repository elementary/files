                                       ...
                                       EelAccessibilityClassInitFn class_init)
{
    ...
    /* Register the type */
    type = g_type_register_static (parent_atk_type, type_name, &tinfo, 0);
...
get_quark_gobject (void)
{
    ...
    if (!quark_accessible_gobject) {
        quark_accessible_gobject = g_quark_from_static_string ("object-for-accessible");
    }
...
{
    g_object_set_qdata (G_OBJECT (data), get_quark_gobject (), NULL);
                                         ...
                                         AtkObject *atk_object)
{
    ...
    if (!ATK_IS_GOBJECT_ACCESSIBLE (atk_object)) {
        g_object_set_qdata_full (object, get_quark_accessible (), atk_object,
                                 (GDestroyNotify)eel_accessibility_destroy);

        g_object_set_qdata (G_OBJECT (atk_object), get_quark_gobject (), object);
    }
...
eel_accessibility_add_simple_text (GType type)
{
    const GInterfaceInfo simple_text_info = {
        (GInterfaceInitFunc) eel_accessibility_simple_text_interface_init,
    ...
    g_type_add_interface_static (type, ATK_TYPE_TEXT, &simple_text_info);
}
...
eel_accessible_text_get_type (void)
{
    ...
    if (!type) {
        ...
        type = g_type_register_static ( G_TYPE_INTERFACE, "EelAccessibleText", &tinfo, 0);
    }
